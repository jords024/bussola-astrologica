...
describe("Database Roles & Admin Operations", () => {
  ...
  it("should check if user has admin role", async () => {
    ...
    expect(db.query).toHaveBeenCalledWith(expect.stringContaining("SELECT EXISTS"), [
      "user-123",
      "admin",
    ]);
  });
  ...
  it("should add a role for a user", async () => {
    ...
    expect(db.query).toHaveBeenCalledWith(expect.stringContaining("INSERT INTO user_roles"), [
      "user-456",
      "admin",
    ]);
  });
