...
import { loginUser, registerUser } from "@/lib/leads.functions";
...
function AuthPage() {
  ...
  const handleLogin = useServerFn(loginUser);
  const handleRegister = useServerFn(registerUser);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      if (mode === "signup") {
        const res = await handleRegister({ data: { email, password } });
        authSession.setSession(res.token, res.user);
      } else {
        const res = await handleLogin({ data: { email, password } });
        authSession.setSession(res.token, res.user);
      }
      goNext();
    } catch (err) {
